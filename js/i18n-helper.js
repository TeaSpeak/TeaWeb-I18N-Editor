...
const apply_filter = () => {
    let count = 0, total = 0;
    const filter_text = ot_text_filter.val().toLowerCase();
    let filter_flags = ot_flags_filter.val().toLowerCase().split(",").map(e => strip(e));
    let filter_flags_negate = ot_flags_filter_negate.val().toLowerCase().split(",").map(e => strip(e));
    if (checkbox_filter_approved[0].checked)
        filter_flags.push(FLAG_VERIFIED);
    else if (!checkbox_filter_approved[0].indeterminate)
...
const _add_flag = (flag) => {
    const li = $(document.createElement("li")).addClass("list-group-item").append($(document.createElement("div")).addClass("input-group").append($(document.createElement("input")).attr("type", "text").addClass("form-control").val(flag), $(document.createElement("div")).addClass("input-group-append").append($(document.createElement("button")).attr("type", "button").addClass("btn btn-outline-danger").html('&times;').on('click', event => {
        li.detach();
...
$(".input-t-flag-new").on('change keyup', event => {
    $(".button-t-flag-new").prop('disabled', !event.target.value);
});
$(".button-t-flag-new").on('click', event => {
    const flag = $(".input-t-flag-new").val();
    console.log(flag);
    if (!flag)
        return;
    _add_flag(flag);
...
const set_current_translation = (trans) => {
...
ta_translated_text.on('change keyup', event => {
    current_translation.translated = ta_translated_text.val();
});
